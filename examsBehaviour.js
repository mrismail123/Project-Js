...
//     localStorage.setItem("CreatedExamInfo", JSON.stringify(examInfo));
...
// console.log( localStorage.getItem("lien de l'examen"));
...
//     let exam = {
//         name: examName,
//         groupDestination : JSON.parse(localStorage.getItem("CreatedExamInfo")).groupDestination,
//         lienDeExamen : localStorage.getItem("lien de l'examen"),
//         questions: questions
...
myForm.addEventListener("submit", async (event) => {
    ...
    localStorage.setItem("CreatedExam",JSON.stringify(examInfo)); // add examinfo in localstorage
    // Modified: Generate a unique examId and include it in the link
    const examId = Math.random().toString(36).substring(2, 8); // e.g., "a1b2c3"
    const uniqueLink = `https://phantomschool.com/examen?examId=${examId}`;
    localStorage.setItem("currentExamId", examId); // Save examId for later use

    let spanLink = document.querySelector("span.linkToExam");
    spanLink.innerHTML = uniqueLink;
    localStorage.setItem("lien de l'examen", uniqueLink);
    let examSettingsSection = document.querySelector(".sections .startExamSettings");
    examSettingsSection.style.display = "flex";
    document.querySelector(".questions").style.display = "block";
    document.querySelector(".exams").style.display = "block";
...
submitExamButton.addEventListener("click", () => {
    ...
    // Modified: Use the examId from localStorage instead of incrementing
    ...
    let exam = {
        examId: examId, // Include examId in the exam object
        name: examName,
        groupDestination: JSON.parse(localStorage.getItem("CreatedExam")).groupDestination,
        lienDeExamen: localStorage.getItem("lien de l'examen"),
        questions: questions
    };
